...
registerScreen("dashboard", (root) => {
  ...
  const el = h("div", { class: "screen dashboard-screen" }, [
    ...
    h("div", { class: "dash-body" }, [
      h("div", { class: "dash-hero" }, [
        h("div", { class: "dash-classname" }, [session.cls?.name ?? "No class"]),
        h("div", { class: "dash-code" }, [
          h("span", { class: "dash-code-label" }, ["JOIN CODE:"]),
          h("span", { class: "dash-code-value" }, [session.cls?.code ?? "—"]),
          h("button", { class: "lib-btn dash-copy" }, ["⧉ COPY"]),
        ]),
        h("div", { class: "dash-role" }, [
          session.cls ? `${session.cls.role.toUpperCase()} · ${session.user.username}` : "STUDENT",
        ]),
      ]),
      ...
      h("div", { class: "dash-actions" }, [
        ...
        h("button", { class: "sticker-btn dash-switch" }, ["⇄ SWITCH CLASS"]),
  ...
  async function loadData() {
    ...
    const info = await cloud.classInfo(cls.id);
    ...
    const quizzes = await cloud.listQuizzes(cls.id);
    if (quizzes.ok && quizzes.data.quizzes) {
      ...
      quizBox.querySelectorAll<HTMLElement>(".dash-quiz-play").forEach((b) => {
        b.addEventListener("click", async () => {
          audio.sfx("select");
          const r = await cloud.fetchQuiz(cls.id, b.getAttribute("data-qid")!);
          if (r.ok && r.data.quiz) {
            const v = validateQuiz(r.data.quiz);
            if (v.ok) {
              saveQuiz(v.quiz, `class:${cls.name}`);
              await startQuiz({ ...v.quiz, source: `class:${cls.name}` });
            } else {
      ...
      quizBox.querySelectorAll<HTMLElement>(".dash-quiz-del").forEach((b) => {
        b.addEventListener("click", async () => {
          const r = await cloud.deleteQuiz(cls.id, b.getAttribute("data-qid")!);
          if (r.ok) {
            audio.sfx("paper");
    ...
    const results = await cloud.classResults(cls.id);
